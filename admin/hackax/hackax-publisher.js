...
function goStage(n){
    ...
    if(
        n > currentStage + 1 &&
        !state.workflow.completed.includes(
            n - 1
        )
    ){
        toast(
            "Complete the current stage before jumping ahead.",
            "error"
        );
...
function validateStage(stage){
    if(stage === 1){
        ...
        if(body.length < 100){
            return {
                ok:false,
                message:
                    "Add at least 100 characters of substantive article content before continuing."
            };
        }
    ...
    if(stage === 2){
        if(
            ...
            !state.media.featured.url
        ){
            return {
                ok:false,
                message:
                    "Upload and process a featured image before continuing."
            };
        }
        ...
        if(
            ...
            return {
                ok:false,
                message:
                    "The featured image must be watermarked before continuing."
            };
        }
    ...
    if(stage === 3){
        if(
            ...
            return {
                ok:false,
                message:
                    "Select a valid HackaX category."
            };
        }
...
function importLegacyRecord(
    ...
    imported.classification.category =
        d.category ||
        "general";

    imported.classification.contentType =
        d.contentType ||
        "update";

    imported.classification.intelTag =
        d.intelTag ||
        d.tag ||
        "GENERAL UPDATE";

    imported.classification.status =
        d.metaStatus ||
        d.meta ||
        "Routine Update";

    imported.classification.destination =
        d.page ||
        "homepage";

    imported.classification.severity =
        d.severity ||
        "Informational";

    imported.classification.verification =
        d.verification ||
        "Verified";

    imported.classification.confidence =
        d.confidence ||
        "Moderate";
    ...
    imported.seo.description =
        d.seoDescription ||
        d.description ||
        d.desc ||
        "";

    imported.seo.canonical =
        d.canonical ||
        "";

    imported.seo.ogPrompt =
        d.ogPrompt ||
        "";

    imported.linking.selected =
        Array.isArray(
            d.relatedArticles
        )
            ? d.relatedArticles
    ...
    imported.workflow.currentStage =
        editMode
            ? 8
            : 1;

    imported.workflow.completed =
        editMode
            ? [1,2,3,4,5,6,7]
            : [];

    imported.workflow.createdAt =
        d.publishedAt ||
        d.date ||
        imported.workflow.createdAt;

    imported.workflow.updatedAt =
        new Date().toISOString();
    ...
    if(editMode){
        ...
        window.HackaXEditSlug =
            imported.article.slug;

        window.HackaXEditDate =
            d.date ||
            "";

        window.HackaXEditPublishedAt =
            d.publishedAt ||
            d.date ||
            "";
    }
...
function sanitizeArticleHTML(html){
    ...
    root.querySelectorAll(
        "script,iframe,object,embed,style,form,input,textarea,button,select,link,meta"
    ).forEach(
        x => x.remove()
    );
...
async function processImage(
    ...
    if(
        ...
        throw new Error(
            "Image is larger than the 25 MB processing limit."
        );
    }
    ...
    if(!ctx){
        throw new Error(
            "Browser image processing is unavailable."
        );
    }
    ...
    if(!blob){
        throw new Error(
            "Unable to create optimized WebP image."
        );
    }
...
async function cloudinaryUpload(
    ...
    if(
        ...
        throw new Error(
            "Cloudinary media settings are not configured."
        );
    }

    const endpoint =
        c.cloudEndpoint ||
        `https://api.cloudinary.com/v1_1/${encodeURIComponent(c.cloudName)}/image/upload`;
...
async function uploadAndProcessImage(
    ...
    if(currentImageJob){
        throw new Error(
            "Another image is currently being processed."
        );
    }
...
async function fetchGithub(path){
    ...
    const url =
        `https://api.github.com/repos/${c.githubRepo}/contents/${String(path).replace(/^\/+/,"")}?ref=${encodeURIComponent(c.githubBranch)}`;
...
async function putGithub(
    ...
    const url =
        `https://api.github.com/repos/${c.githubRepo}/contents/${String(path).replace(/^\/+/,"")}`;
    ...
    if(!response.ok){
        throw new Error(
            data.message ||
            `GitHub publish failed (${response.status}).`
        );
    }
...
async function loadArticles(){
    try{
        ...
        console.warn(
            "HackaX article corpus unavailable:",
            e
        );
...
function renderLinkSuggestions(){
    ...
    if(!candidates.length){
        box.innerHTML =
            `<div class="empty-state">No high-confidence contextual relationship was found. The engine will not force an unrelated link.</div>`;
...
function automaticallyLinkArticle(){
    ...
    if(!candidates.length){
        toast(
            "No safe contextual links found.",
            "info"
        );
...
function autoDescription(){
    ...
    toast(
        "Meta description generated from the actual article.",
        "success"
    );
}
...
function autoSEO(){
    ...
    state.seo.title =
        `${state.article.title} | Dyve HackaX`
            .slice(
                0,
                70
            );
    ...
    state.seo.ogPrompt =
        ...
        }; dark forensic environment, emerald network glow, technical intelligence aesthetic, realistic, high contrast, premium publication image, no text, no logos.`;
    ...
    if(quality){
        ...
        quality.textContent =
            !d
                ...
                : d.length >= 140 &&
                  d.length <= 160
                    ? "Strong search snippet length"
                    : d.length >= 100
                        ? "Usable, but can be improved"
                        : "Too short for a strong search snippet";
    }
}
...
function seoChecks(){
    ...
    return [
...

        {
            ...
            message:
                `Title length: ${topic.title.length} characters.`
        },

        {
            ...
            message:
                `Meta description: ${state.seo.description.length}/160 characters.`
        },
...

        {
            ...
            message:
                `Heading structure: ${h2} H2 sections.`
        },
...

        {
            ...
            message:
                f?.alt
                    ? "Featured image has contextual alt text."
...

        {
            ...
            message:
                f?.seoName
                    ? "SEO image filename is present."
                    : "SEO image filename is missing."
        },

        {
            ...
            message:
                f?.webp
                    ? "Featured image is WebP."
                    : "Featured image is not confirmed as WebP."
        },

        {
            ...
            message:
                f?.watermarked
                    ? "HackaX watermark is permanently embedded."
                    : "HackaX watermark is missing."
        },
...
function technicalChecks(){
    ...
    const checks = [
...

        {
            ...
            message:
                "Slug is normalized for a clean URL."
        },

        {
            ...
            message:
                "Article route ends in .html for static SEO publishing."
        },

        {
            ...
            message:
                state.media.featured?.url
                    ? "Featured image URL is available."
                    : "Featured image is missing."
        },

        {
            level:
                state.media.featured?.width &&
                state.media.featured?.height
                    ? "success"
                    : "warning",

            message:
                "Featured image intrinsic dimensions are stored."
        },
        ...
        {
            level:
                state.linking.applied >= 1 ||
                countExistingInternalLinks() >= 1
                    ? "success"
                    : "warning",
...

        {
            ...
            message:
                existingArticles.length
                    ? `${existingArticles.length} HackaX records loaded for discovery.`
    ...
    root.innerHTML = `
        ...
        <div class="review-status ${
            ...
            <span>
                ${
                    errors
                        ? "Resolve the blocking checks before publishing."
                        : "The publisher can generate the article, index record and sitemap updates."
                }
            </span>
...
<meta
    ...
    content="Dyve HackaX Intelligence Unit"
>
...
<link
    rel="icon"
    href="${attr(c.siteUrl)}/favicon.ico"
>

<link
    href="https://fonts.googleapis.com/css2?family=Audiowide&family=Inter:wght@400;500;600;700;800&family=JetBrains+Mono:wght@400;500&display=swap"
    rel="stylesheet"
>

<link
    rel="stylesheet"
    href="${attr(pathDepth)}/article.css"
>
...
<section class="hx-cta">
    ...
    <div class="hx-cta-card">

        <h2>
            Stay ahead of active threats
        </h2>

        <p>
            Track breach signals, threat actors,
            ransomware activity and underground
            intelligence with Dyve HackaX.
        </p>
...
<footer class="hx-footer">
    ...
    <div class="hx-legal">
        ...
        <a
            href="${attr(pathDepth)}/privacy-policy/index.html"
        >
            Privacy
        </a>

        <a
            href="${attr(pathDepth)}/terms-of-service/index.html"
        >
            Terms
        </a>
...
function buildRecord(){
    ...
    const keywords =
        unique([
            ...
            ...state.article.tags.flatMap(
                meaningfulTokens
            ),

            ...state.classification.domains.flatMap(
                meaningfulTokens
            ),
...
async function publishArticle(){
    ...
    if(
        ...
        toast(
            "Configure GitHub in HackaX Settings first.",
            "error"
        );
    ...
    try{
        toast(
            "Checking the live HackaX article index..."
        );
        ...
        toast(
            "Publishing article page to GitHub..."
        );
        ...
        await putGithub(
            ...
            `${
                window.HackaXEditMode
                    ? "update"
                    : "chore"
            }: HackaX article index`,
            db &&
            db.sha
        );

        toast(
            "Updating HackaX XML sitemaps..."
        );
        ...
        toast(
            window.HackaXEditMode
                ? "HackaX Intel updated successfully."
                : "HackaX Intel published successfully.",
            "success"
        );
...
function aiGrammar(){
    ...
    toast(
        count
            ? `Fixed ${count} common writing issues.`
...
function replaceSelection(
    ...
    if(
        ...
        toast(
            "Select text in the article first.",
            "error"
        );
...
function aiShorter(){
    replaceSelection(
        text =>
            text
                .replace(
                    /\b(very|really|quite|actually|basically|just|simply)\b/gi,
                    ""
                )
                .replace(
                    /in order to/gi,
...
function aiLonger(){
    replaceSelection(
        text =>
            `${text.trim()} This development should be assessed alongside the broader threat landscape, operational exposure, relevant indicators and the evidence currently available to defenders.`
    );

    saveState(true);

    toast(
        "Selected text expanded.",
        "success"
    );
}
...
function aiSEO(){
    ...
    if(!terms.length){
        toast(
            "Not enough article content to extract SEO topics.",
            "error"
        );
    ...
    toast(
        "SEO topic tags generated from the article.",
        "success"
    );
}
...
<p>This section summarizes the most important findings, observed indicators and operational significance of the ${esc(label.toLowerCase())} development.</p>
<h2>Why It Matters</h2>
<p>The development should be assessed alongside the broader threat landscape, affected infrastructure, exposure patterns and the evidence currently available to defenders.</p>
<h2>Recommended Actions</h2>
<ul>
<li>Validate affected systems, identities or infrastructure.</li>
<li>Review relevant logs and threat indicators.</li>
<li>Increase monitoring where evidence supports elevated risk.</li>
<li>Document observed indicators and preserve relevant evidence.</li>
</ul>`;
...
function customAI(){
    ...
    if(
        ...
        toast(
            'Try "uppercase", "lowercase", "summary" or "bold".',
            "info"
        );
                                ...
                                input.onchange =
                                    async() => {
                                        for(
                                            ...
                                            try{
                                                toast(
                                                    "Processing article image..."
                                                );

                                                const meta =
                                                    await uploadAndProcessImage(
                                                        file,
                                                        "body"
                                                    );
    ...
    document
        ...
        ?.addEventListener(
            ...
            async e => {
                ...
                try{
                    toast(
                        "Compressing, converting to WebP and embedding HackaX watermark..."
                    );
                    ...
                    toast(
                        "Featured image processed and uploaded.",
                        "success"
                    );
    ...
    document
        ...
        ?.addEventListener(
            ...
            () => {
                ...
                toast(
                    "Review refreshed from current publisher state.",
                    "success"
                );
            }
        );
    ...
    document
        ...
        ?.addEventListener(
            ...
            () => {
                if(
                    confirm(
                        "Start a new HackaX publication? The current local publisher state will be cleared."
                    )
                ){
                    localStorage.removeItem(
                        STORAGE.publisher
...
async function checkGitHub(){
    ...
    try{
        const response =
            await fetch(
                `https://api.github.com/repos/${c.githubRepo}`,
                {
                    headers:{
                        Authorization:
                            `Bearer ${c.githubToken}`,
